...
![{A60D6436-8AAD-4AEC-B063-56A87F96713C}](https://github.com/user-attachments/assets/66470ca7-f6f3-4290-ae09-3d3904102dfb)

Добавляем Drow.io

### Задание 38

Когда мы переключаемся на предыдущую версию пропадает диаграмма Draw.io

### Задание 39


![{38285209-973D-4AF6-8C02-76B760EBAEB0}](https://github.com/user-attachments/assets/39d91bfe-e09f-40b1-9e82-568a1d63456b)

Это расширение поддерживает форматы .wsd, .pu, .pulm, .plantuml, .iuml
### Задание 40


![{79377E69-8573-45F3-B1CE-6B2B9BC3E57F}](https://github.com/user-attachments/assets/88638ebe-877c-4349-aa31-a9385a6c7754)
...
Команда docker run:
docker run – это основная команда для запуска контейнера Docker.
Параметры:
-d (или --detach) – запускает контейнер в фоновом режиме (демон). Это позволяет продолжить работу с терминалом после старта контейнера.
-p 8080:8080 – указывает, что порт 8080 внутри контейнера будет доступен снаружи также через порт 8080. Формат записи: <внешний_порт>:<внутренний_порт>. В данном случае внешний и внутренний порты совпадают.
Образы:
plantuml/plantuml-server:jetty и plantuml/plantuml-server:tomcat — это образы Docker, которые вы используете для запуска PlantUML-сервера.
PlantUML — инструмент для создания диаграмм на основе текстовых описаний.
Эти два образа отличаются только тем, какой веб-сервер используется для работы PlantUML:
Jetty — легкий и быстрый сервер приложений Java.
Tomcat — более популярный и мощный сервер приложений для Java.
Таким образом, обе команды делают одно и то же: запускают PlantUML-сервер в контейнере Docker, но используют разные версии сервера — Jetty и Tomcat соответственно.

### Задание 42


![377089101-83d4845f-d194-42a0-900b-8e109c08cdf7](https://github.com/user-attachments/assets/265bc15c-01cb-427e-bd23-7ea8cddf3f57)
